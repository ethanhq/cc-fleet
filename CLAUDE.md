...
`cc-fleet` is a Go CLI (+ one Claude Code skill) that runs third-party LLM providers
(DeepSeek, GLM, Kimi, Qwen, MiniMax — anything with an Anthropic-compatible API) as **real
Claude Code workers**. The core trick: a provider worker is a genuine `claude` process whose
LLM backend is swapped by launching it with `--settings <provider-profile>.json` (which sets
`ANTHROPIC_BASE_URL` + an `apiKeyHelper`) and `--model <provider-model-id>`. The main session's
...
- **Teammate (lane 1)** — `internal/spawn`. Launches a long-lived provider `claude` in a tmux
...
To make a provider worker behave like a native teammate, cc-fleet must launch `claude` with the
...
-u ANTHROPIC_AUTH_TOKEN` + the provider profile + model.
...
The provider API key must never reach `env`, `argv`, `ps aux`, or shell history. The mechanism:
`profile.GenerateForProvider` writes `~/.claude/profiles/<provider>.json` with
`apiKeyHelper: "<cc-fleet-abs-path> keyget <provider>"`. Claude Code invokes that at runtime;
...
(`<provider>.keys.json`) with per-key enable/disable and rotation (`off`/`round_robin`/`random`).

### Config & on-disk layout (`internal/config`)

- `~/.config/cc-fleet/providers.toml` — single source of truth users edit (schema `version = 1`,
...
- `WithProvidersConfigLock` — global `<ConfigDir>/.cc-fleet-providers.lock`. Guards the
  `providers.toml` load→mutate→save cycle (`add`/`edit`/`remove`).

**Lock ordering (no cycles):** providers-config OUTERMOST, then team, then server INNERMOST.
...
Provider/team/agent names flow into file paths (`filepath.Join` → traversal risk) and into the
`apiKeyHelper` shell string (`<bin> keyget <name>` → injection risk). Validate **before** use:
`ValidateProviderName` / `ValidateTeamName` / `ValidateMemberName`, and `EnsureUnderRoot` to
