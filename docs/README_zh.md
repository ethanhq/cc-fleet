![cc-fleet — 为 Claude Code 的 Dynamic Workflow、Agent Team、Subagent 接入任意第三方模型](assets/cc-fleet-banner.png)
...
**Codex 插件** — 要从 Codex 会话驱动 cc-fleet,装完上面的 cc-fleet 后运行 `codex plugin marketplace add ethanhq/cc-fleet`,然后 `codex plugin add cc-fleet`。
...
cc-fleet 也能插进 **OpenAI Codex**:一个 Codex 插件让 Codex 会话扇出同样的 Provider worker — 一次性或批量 **subagent**(`cc-fleet subagent`)与多阶段 JS **workflow**(`cc-fleet workflow`),每个都是跑在 Provider 模型上的真实 `claude` 进程。安装:`codex plugin marketplace add ethanhq/cc-fleet`,然后 `codex plugin add cc-fleet`(`cc-fleet` 与 `claude` 二进制单独安装)。Teammate lane 仍只在 Claude Code 内;从 Codex 你得到 subagent 与 workflow 扇出。
